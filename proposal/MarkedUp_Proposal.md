...
- **Admin analytics view:** A protected `/admin` page (restricted to my email) showing per-user upload counts, total uploads over time, and active user counts — basic usage analytics to see how people are interacting with the site.
  ...
  > ✅ Implemented. A protected `/admin` route is live, guarded by the `ADMIN_EMAILS` env var — accounts not in that list receive a 403. The page shows: (1) a KPI strip with total users, total uploads, statements saved, save rate (uploads → saves ratio), and active users this week; (2) a site-wide upload activity area chart; (3) a new-user signups area chart; (4) a searchable, scrollable user management table. All charts support 7d / 30d / All time range toggles. Clicking a user row expands a drill-down panel with their personal upload history chart, statement count, and estimated LLM cost. Admin actions include resetting a user's daily upload limit and hard-deleting a user and all their data. The page auto-refreshes whenever a new statement is uploaded. Token counts (`input_tokens`, `output_tokens`) are stored per upload in the `usage` table for accurate cost estimation. `/auth/me` returns an `is_admin` flag so the frontend conditionally shows the Admin nav link.
  >
  > **Timezone support (part of Admin):** All time-based data on the Admin page — chart date buckets, "uploads today" counts, "last active" dates, "signed up" timestamps, and chart range boundaries — are computed and displayed in the user's selected timezone rather than UTC. Timezone preference is set via a **Settings** dropdown in the NavBar (gear icon, replaces the old standalone Sign out button). The preference is saved to `localStorage` and defaults to the browser's auto-detected timezone so it works correctly for any user without configuration. The backend accepts a `tz` query parameter (IANA timezone string) on all `/admin/*` endpoints and uses PostgreSQL's `AT TIME ZONE` to re-bucket dates accordingly. A `logged_at TIMESTAMPTZ` column was added to the `usage` table so upload timestamps are stored with full precision; a `COALESCE(logged_at, date::timestamptz)` fallback handles rows that existed before this column was added.
