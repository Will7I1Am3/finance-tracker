...
- **Rate limiting:** Per-user upload counter stored in Postgres — ✅ Implemented. `POST /statements/upload` in `backend/routers/statements.py` checks the `usage` table before calling the LLM and increments the count after. Cap is set to 10 uploads per user per day via the `DAILY_UPLOAD_LIMIT` env var. The same daily quota also gates `/redact/preview` and `/redact/apply` — users who have exhausted their limit cannot load a PDF into the redaction canvas at all.
...
4. **Rate Limiter** — Before making a Claude API call, the backend checks how many uploads the user has made today against a `usage` table in Postgres. If they have hit the daily cap (default 10), the upload is blocked and the user is told when the limit resets. If not, the count is incremented and the upload proceeds.
   ...
   > ✅ Implemented as written. Logic lives in `POST /statements/upload` in `backend/routers/statements.py`. The cap is configurable via the `DAILY_UPLOAD_LIMIT` env var. The current count is also shown in the upload modal header with a color warning at ≤2 remaining. The same daily quota also gates `POST /redact/preview` and `POST /redact/apply` so users cannot start the redaction canvas flow once their limit is exhausted.
...
- **Admin analytics view:** A protected `/admin` page (restricted to my email) showing per-user upload counts, total uploads over time, and active user counts — basic usage analytics to see how people are interacting with the site.
  ...
  > ✅ Implemented. Backend logic is in `backend/routers/admin.py`; frontend page is `frontend/src/pages/Admin.jsx`. A protected `/admin` route is live, guarded by the `ADMIN_EMAILS` env var — accounts not in that list receive a 403. The page shows: (1) a KPI strip with total users, total uploads, statements saved, save rate (uploads → saves ratio), and active users this week; (2) a 2×2 chart grid — site-wide upload activity, new-user signups, daily LLM cost, and cumulative LLM cost; (3) a searchable, scrollable user management table. All charts support 7d / 30d / All time range toggles. Clicking a user row expands a drill-down panel with their personal upload history chart, statement count, and estimated LLM cost. Admin actions include resetting a user's daily upload limit and hard-deleting a user and all their data. The page auto-refreshes whenever a new statement is uploaded. Token counts (`input_tokens`, `output_tokens`) are stored per upload in the `usage` table; cost is estimated using Claude Haiku's actual token pricing when available, falling back to a flat per-upload estimate for older rows. `/auth/me` returns an `is_admin` flag so the frontend conditionally shows the Admin nav link.
  >
  > **Timezone support (part of Admin):** The database stores all dates in UTC. Without timezone conversion, activity on the admin charts would be bucketed by UTC day, which shows the wrong date for users in other timezones. The admin page accepts an IANA timezone string and converts timestamps to the user's local date before bucketing. Timezone preference is managed in `frontend/src/SettingsContext.jsx`, surfaced via a settings dropdown in `frontend/src/components/NavBar.jsx`, saved to `localStorage`, and defaults to the browser's detected timezone.
